...
const checkWin = () => {
  ...
  if (win === 100) {
    gameOver = true;
    emoji.src = '/img/Sunglasses Emoji.png';
    stopTimer();
  }
};
...
allCells.forEach((cell) => {
  cell.addEventListener('click', (event) => {
    ...
    if (!gameOver) {
      ...
      if (event.currentTarget.hasAttribute('data-mine') && !flagged) {
        ...
        emoji.src = '/img/Astonished Face Emoji.png';
        stopTimer();
        gameOver = true;
      } else if (!flagged) {
        reveal(rowsArray, cellRow, cellColumn);
  ...
  cell.addEventListener('mousedown', () => {
    if (!gameOver) { emoji.src = '/img/Surprised Face Emoji.png'; }
  });

  cell.addEventListener('mouseup', () => {
    if (!gameOver) { emoji.src = '/img/Slightly Smiling Face Emoji.png'; }
  });
});
