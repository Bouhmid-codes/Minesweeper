...
const checkWin = () => {
  ...
  if (win === 100) {
    gameOver = true;
    emoji.innerHTML = '<i class="fa-regular fa-face-grin-stars"></i>';
    stopTimer();
  }
};
...
allCells.forEach((cell) => {
  cell.addEventListener('click', (event) => {
    ...
    if (!gameOver) {
      ...
      if (event.currentTarget.hasAttribute('data-mine') && !flagged) {
        ...
        emoji.innerHTML = '<i class="fa-regular fa-face-dizzy"></i>';
        stopTimer();
        gameOver = true;
      } else if (!flagged) {
        reveal(rowsArray, cellRow, cellColumn);
  ...
  cell.addEventListener('mousedown', () => {
    if (!gameOver) { emoji.innerHTML = '<i class="fa-regular fa-face-surprise"></i>'; }
  });

  cell.addEventListener('mouseup', () => {
    if (!gameOver) { emoji.innerHTML = '<i class="fa-regular fa-face-smile"></i>'; }
  });
});
